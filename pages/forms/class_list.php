...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Ln(3);
        $que3 = mysqli_num_rows(mysqli_query(
            $db,
    ...
    "
        ...
        $qwer = mysqli_fetch_array(mysqli_query(
            $db,
            "SELECT * 
    FROM tbl_schedules 
...
while ($row = mysqli_fetch_array($que)) {
    $qwe = mysqli_query($db, "SELECT * FROM tbl_schoolyears LEFT JOIN tbl_year_levels ON tbl_year_levels.year_id = tbl_schoolyears.year_id where stud_id = '$row[stud_id]' AND sem_id = '$_SESSION[active_sem]' AND ay_id = '$_SESSION[active_acad]' AND tbl_schoolyears.remark = 'Approved'");
